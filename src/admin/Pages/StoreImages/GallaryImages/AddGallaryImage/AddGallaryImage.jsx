...
function AddGallaryImage() {
    ...
    const addGallery = async (e) => {
        ...
        if (images.length === 0) {
            toast.error('Please select gallery images', { position: 'top-center' });
            setLoading(false);
            return
        } else {
            ...
            try {
                ...
                const response = await fetch(`${config.backEndBaseUrl}api/store/gallery/add/${uid}`, {
                    method: 'POST',
                    headers: {
                ...
                if (result.status) {
                    ...
                    navigate(`/admin/store/images/${uid}?tab=2`);
                } else {
...
export default AddGallaryImage