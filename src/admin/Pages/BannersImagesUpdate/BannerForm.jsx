...
function BannerForm() {
    ...
    const [bannerYoutubeLink, setBannerYoutubeLink] = useState('');
    ...
    const getBannerImageDetails = async () => {
        try {
            const response = await fetch(`${config.backEndBaseUrl}api/banner/images/${uid}`, {
                headers: {
            ...
            if (!result.status) {
                ...
                setBannerYoutubeLink(result.data.youtube_link);
    ...
    const updateBannerImage = async (event) => {
        ...
        if (images.length === 0 && !prefetchedImage) {
            ...
            const bannerData = {
                ...
                youtube_link: bannerYoutubeLink,
            ...
            try {
                const response = await fetch(`${config.backEndBaseUrl}api/banner/images/update/${uid}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (result.status) {
                    ...
                    navigate(`/admin/banner/image/${bannerId}`);
                } else {
    ...
    const youtubeLinkChange = (event) => {
        setBannerYoutubeLink(event.target.value);
    }
            ...
            <div className="wg-box">
                <form className="form-new-product form-style-1" onSubmit={updateBannerImage}>
                    ...
                    <fieldset className="name">
                        ...
                        <input className="flex-grow" type="text" placeholder="Banner Image Link" value={bannerYoutubeLink} onChange={youtubeLinkChange} />
                    </fieldset>
                    <fieldset>
