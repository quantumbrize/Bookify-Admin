...
function UpdateProductImageForm() {
    ...
    const getBannerImageDetails = async () => {
        try {
            const response = await fetch(`${config.backEndBaseUrl}api/product/image/${uid}`, {
                headers: {
    ...
    const updateBannerImage = async (event) => {
        ...
        if (images.length === 0 && !prefetchedImage) {
            ...
            try {
                const response = await fetch(`${config.backEndBaseUrl}api/product/image/update/${uid}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
